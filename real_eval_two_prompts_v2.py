"""Test: Prompt 1 (Freeform CoT) vs Prompt 2 (Structured CoT) on 30 problems.

Same setup as real_eval_two_prompts.py but tripled to 30 logic/reasoning
problems (6 per category across 5 categories). This widens n enough that
Caliber can resolve the verdict if there is a real effect.

Run:
    python -X utf8 real_eval_two_prompts_v2.py
"""
...
# 30 logic / reasoning problems, 6 per category.
PROBLEMS: list[tuple[str, str, str]] = [
    ...
    ("All birds lay eggs. Some animals that lay eggs are reptiles. Therefore some birds are reptiles. "
     "Is this argument logically valid? Reply with only 'yes' or 'no'.",
     "no", "syllogism"),
    ("All squares are rectangles. All rectangles have four sides. Do all squares have four sides? "
     "Reply with only 'yes' or 'no'.",
     "yes", "syllogism"),
    ("All cats are mammals. All mammals are animals. Are all cats animals? Reply with only 'yes' or 'no'.",
     "yes", "syllogism"),
    ("Some doctors are tall. All tall people need long beds. Therefore some doctors need long beds. "
     "Is this argument logically valid? Reply with only 'yes' or 'no'.",
     "yes", "syllogism"),
    ("All apples are fruits. No fruits are vegetables. Therefore no apples are vegetables. "
     "Is this argument logically valid? Reply with only 'yes' or 'no'.",
     "yes", "syllogism"),
    ...
    ("Five books are stacked from bottom to top: Math, Science, History, Art, Biology. "
     ...
     "History", "spatial"),
    ("In a line from left to right: Tom, Mary, Sue, Joe. Who is third from the left? Reply with just the name.",
     "Sue", "spatial"),
    ("Four cards arranged left to right: Ace, King, Queen, Jack. Which card is second from the right? "
     "Reply with just the card name.",
     "Queen", "spatial"),
    ("Anna is to the left of Ben. Ben is to the left of Carl. Carl is to the left of Dan. "
     "Who is on the far right? Reply with just the name.",
     "Dan", "spatial"),
    ("A building has 5 floors numbered 1 (bottom) to 5 (top). Which floor is directly above floor 3? "
     "Reply with just the floor number.",
     "4", "spatial"),

    # ----------------- counting / combinatorics (6) -----------------
    ...
    ("How many distinct 2-letter sequences can be formed from the letters A, B, C if letters can repeat?",
     "9", "counting"),
    ("A bag has 4 red marbles and 6 blue marbles. How many marbles total?",
     "10", "counting"),
    ("How many days are in 3 weeks?",
     "21", "counting"),
    ("A pizza is cut into 8 slices. Tom eats 3 slices and Sara eats 2 slices. How many slices are left?",
     "3", "counting"),
    ...
    ("What comes next: 2, 4, 8, 16, ?",
     "32", "sequence"),
    ("What number is missing in this triangular number sequence: 1, 3, 6, 10, ?, 21",
     "15", "sequence"),
    ("What comes next: 5, 10, 15, 20, ?",
     "25", "sequence"),
    ("What comes next in the Fibonacci sequence: 0, 1, 1, 2, 3, 5, ?",
     "8", "sequence"),

    # ----------------- conditional / logical (6) -----------------
    ...
    ("If a number is divisible by 4, it is also divisible by 2. The number 6 is divisible by 2. "
     ...
     "no", "conditional"),
    ("If today is Monday, tomorrow is Tuesday. Today is Wednesday. Is tomorrow Tuesday? "
     "Reply with only 'yes' or 'no'.",
     "no", "conditional"),
    ("If a person studies, they pass the test. Mary studied. Did Mary pass the test? "
     "Assume the premise is true. Reply with only 'yes' or 'no'.",
     "yes", "conditional"),
    ("If you study hard, you will pass. Jim did not pass. Did Jim study hard? "
     "Assume the premise is true. Reply with only 'yes' or 'no'.",
     "no", "conditional"),
    ("If a shape is a triangle, it has 3 sides. A square has 4 sides. Is a square a triangle? "
     "Reply with only 'yes' or 'no'.",
     "no", "conditional"),
]
...
def run_arm(template: str, label: str) -> ArmResult:
    ...
    for i, (q, correct, _cat) in enumerate(PROBLEMS):
        ...
        print(f"  [{i + 1:2d}/{len(PROBLEMS)}] {status}  correct={correct:>8}  phi3 said={guess:>10}  | {q_short}")
    print(f"  accuracy: {scores.mean():.0%}  ({int(scores.sum())}/{len(scores)})  elapsed: {time.time() - t0:.1f}s")
...
def main() -> int:
    ...
    print(f"Caliber two-prompt comparison v2 — {MODEL} via Ollama")
    print(f"Task: {len(PROBLEMS)} logic/reasoning problems "
          f"({len({p[2] for p in PROBLEMS})} categories, "
          f"{len(PROBLEMS) // len({p[2] for p in PROBLEMS})} per category)")
    ...
    if verdict.verdict == "BETTER":
        ...
        winner = None; loser = None
        print(f"Verdict: INCONCLUSIVE — at n={len(arm1.scores)}, we still cannot call "
              "one prompt better than the other.")
    ...
    if explanation.top_improvements:
        ...
        for ex in explanation.top_improvements:
            ...
            print(f"            P1 said wrong | P2 said right")
    if explanation.top_regressions:
        ...
        for ex in explanation.top_regressions:
            ...
            q_short = ex.input if ex.input and len(ex.input) <= 60 else (ex.input[:57] + "..." if ex.input else "")
            print(f"  [#{ex.index:2d}] {cat:<12}  {q_short}")
            print(f"            P1 said right | P2 said wrong")
    ...
    if verdict.verdict in ("BETTER", "WORSE"):
        ...
        if verdict.verdict == "BETTER" and len(explanation.top_improvements) >= 2:
            ...
            # Invert: ask the judge to read cases where Prompt 1 won
            ...
            print(f"\nAsking {provider.name}/{provider.model} to analyze "
                  f"{min(3, len(inverted.top_improvements))} cases where Prompt 1 won...")
            t0 = time.time()
        ...
        print(f"\nSkipping hypothesis — verdict is {verdict.verdict}, no winner to explain.")
